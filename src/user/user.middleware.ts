...
export const validateUserData = async (
  ...
  // 验证必填数据
  if (!name) return next(new Error('MAME_IS_REQUIRED'));
 ...
 * 验证更新用户数据
 */
export const validateUpadateUserData = async (
  request: Request,
  response: Response,
  next: NextFunction,
) => {
  ...
  try {
    ...
    // 验证用户密码是否匹配
    const meatched = await bcrypt.compare(validate.password, user.password);
    ...
    // 处理用户更新密码
    if (update.password) {
      const meatched = await bcrypt.compare(update.password, user.password);
